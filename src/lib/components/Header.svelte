...
<div
	class="w-full border-b border-yellow-700/50 bg-yellow-900/40 px-4 py-2 text-center text-sm text-yellow-200"
>
	⚠️ Development Build: For testing purposes only. Features may change or be unstable.
</div>
	...
	<div class="flex items-center gap-3">
		{#if $user}
			<div class="relative" transition:fade>
				...
				>
					<div
						class="relative grid h-8 w-8 place-items-center rounded-full bg-white/10 text-sm font-medium"
					>
						{String(($user as any).email ?? 'U')
							.slice(0, 1)
							.toUpperCase()}
						<span
							class="absolute right-0 bottom-0 h-3 w-3 rounded-full border border-neutral-900 bg-emerald-500"
							title="Online"
						></span>
					</div>
				...
				{#if menuOpen}
					<div
						class="absolute right-0 mt-2 w-48 overflow-hidden rounded-lg border border-white/10 bg-neutral-900/95 shadow-xl"
						transition:scale={{ duration: 120 }}
					>
						<button
							class="flex w-full items-center gap-2 px-3 py-2 text-sm text-gray-100 hover:bg-white/10"
						...
						<button
							class="flex w-full items-center gap-2 px-3 py-2 text-sm text-red-200 hover:bg-red-500/20"
							on:click={logout}
						>
							Sign out
						</button>
