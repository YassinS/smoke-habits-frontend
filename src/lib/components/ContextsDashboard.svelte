<script lang="ts">
  ...
  import { fetchSmokeContexts, createSmokeContext, updateSmokeContext } from '$lib/api';
...
<div class="space-y-6">
  ...
  <Card className="p-4">
    <div class="grid gap-3 md:grid-cols-[2fr_1fr_auto] md:items-end">
      <div class="flex flex-col gap-1">
        <label class="text-xs uppercase tracking-wide text-gray-300" for="contextLabel">Label</label>
        <input
          id="contextLabel"
      ...
      <div class="flex flex-col gap-1">
        <span class="text-xs uppercase tracking-wide text-gray-300" id="contextColorLabel">Accent color</span>
    ...
    {#if error}
      <div class="mt-3 rounded border border-red-500/30 bg-red-500/10 p-2 text-sm text-red-300">{error}</div>
    {/if}
  </Card>
  ...
  {:else if !contexts.length}
    <Card className="p-6 text-center text-sm text-gray-300">No contexts yet. Create one above to begin marking moments.</Card>
  {:else}
    <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
      {#each contexts as ctx}
        <Card className="flex items-center justify-between gap-4 p-4">
          <div class="flex items-center gap-3">
            <span class="h-10 w-10 rounded-full border border-white/10" style={`background:${ctx.colorUI};`}></span>
            <div>
          ...
          <button
            class="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs uppercase tracking-wide text-white hover:bg-white/15"
